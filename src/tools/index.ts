...
export class PremiereProTools {
  ...
  private async exportSequence(sequenceId: string, outputPath: string, presetPath?: string, format?: string, quality?: string, resolution?: string): Promise<any> {
    try {
      // presetPath MUST be an absolute filesystem path to a real .epr file.
      // The previous default of bare names like "H.264" / "ProRes" silently failed
      // inside encodeSequence — Premiere requires a path the encoder can resolve.
      if (!presetPath) {
        return {
          success: false,
          error: "presetPath required: pass an absolute path to a .epr file (e.g. /Applications/Adobe Media Encoder 2026/.../01 - Match Source - High bitrate.epr). Bare format names are not accepted by encodeSequence.",
          sequenceId,
          outputPath
        };
      }
      const bridgeResult = await this.bridge.renderSequence(sequenceId, outputPath, presetPath);
      // Propagate real success/failure from JSX. Do NOT wrap with hardcoded success:true.
      if (bridgeResult && bridgeResult.success === false) {
        return {
          success: false,
          error: bridgeResult.error || "Bridge reported failure without error",
          sequenceId,
          outputPath,
          presetPath,
          bridgeResult
      ...
      return {
        ...
        queued: bridgeResult?.queued ?? true,
