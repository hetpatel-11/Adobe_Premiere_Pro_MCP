...
export class PremiereProTools {
    ...
    async addMarker(_sequenceId, time, name, comment, color, duration) {
        const script = `
      try {
    ...
    async deleteMarker(_sequenceId, markerId) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async updateMarker(_sequenceId, markerId, updates) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async addTrack(_sequenceId, trackType, _position) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async deleteTrack(_sequenceId, trackType, trackIndex) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async renameTrack(_sequenceId, trackType, trackIndex, newName) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async lockTrack(_sequenceId, trackType, trackIndex, locked) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async toggleTrackVisibility(_sequenceId, trackIndex, visible) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async normalizeAudio(_clipId, _targetLevel) {
        return {
            success: false,
    ...
    async audioDucking(_backgroundClipId, _dialogueClipId, _duckAmount, _fadeTime) {
        return {
            success: false,
    ...
    async extractAudio(_clipId, _outputPath) {
        return {
            success: false,
    ...
    async linkAudioVideo(clipId, linked) {
        const script = `
      try {
    ...
    async applyAudioEffect(clipId, effectName, parameters) {
        return await this.applyEffect(clipId, effectName, parameters);
    }
    // Nested Sequences
    async createNestedSequence(_clipIds, _name) {
        return {
            success: false,
    ...
    async unnestSequence(_nestedSequenceClipId) {
        return {
            success: false,
    ...
    // Additional Clip Operations
    async duplicateClip(clipId, offset) {
        const script = `
      try {
        var clip = app.project.getClipByID(${JSON.stringify(clipId)});
        if (clip) {
    ...
    async reverseClip(clipId, maintainAudioPitch) {
        return await this.speedChange(clipId, -100, maintainAudioPitch !== false);
    }
    async enableDisableClip(clipId, enabled) {
        const script = `
      try {
        var clip = app.project.getClipByID(${JSON.stringify(clipId)});
        if (clip) {
    ...
    async replaceClip(_clipId, _newProjectItemId, _preserveEffects) {
        return {
            success: false,
    ...
    async slipClip(_clipId, _slipAmount) {
        return {
            success: false,
    ...
    async slideClip(_clipId, _slideAmount) {
        return {
            success: false,
    ...
    // Project Settings
    async getSequenceSettings(_sequenceId) {
        const script = `
      try {
        var sequence = app.project.activeSequence;
        if (!sequence) {
    ...
    async setSequenceSettings(_sequenceId, _settings) {
        return {
            success: false,
    ...
    async getClipProperties(clipId) {
        const script = `
      try {
        var clip = app.project.getClipByID(${JSON.stringify(clipId)});
        if (clip) {
    ...
    async setClipProperties(_clipId, _properties) {
        return {
            success: false,
    ...
    // Render Queue
    async addToRenderQueue(sequenceId, outputPath, presetPath, _startImmediately) {
